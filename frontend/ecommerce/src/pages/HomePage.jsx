...
const HomePage = () => {
	...
	useEffect(() => {
		if (currentUser && currentUser.user_id) { // Asegúrate de que currentUser y su id estén disponibles
			fetchproductosDestacados(currentUser.user_id); 
			console.log("ID del usuario:", currentUser.user_id);
			
			 // Pasar el ID del usuario a la función
		}

		// Si es necesario, también puedes pasar el ID en la función fetchproductosDestacados
		 // Aquí puedes agregar parámetros si lo necesitas también

	}, [ fetchproductosDestacados, currentUser]); // Dependencias
